...
<div class="row-fluid">
	<form id="userEdit" action="/user/edit/{{$user->id}}" method="POST">
		<div style="margin-left:0" class="control-group span12">
	        <label class="control-label">Username:</label>
	        <div class="controls">
	            <input type="text" name="username" id="username" placeholder="username" disabled="disabled" value="{{$user->email}}"/>
	        </div>
	    </div>
	    <div style="margin-left:0" class="control-group span12">
	    ...
	    <?php if(Auth::user()->can('userCreation') and !(Auth::user()->id == $user->id)) {?>
			<div style="margin-left:0" class="control-group span12">
	    ...
	    <?php } ?>
	    {{Form::token()}}
	    <button onClick="$('userEdit').submit()">Submit</button>
	    <button onclick="window.location='/account/manage/">Cancel</button>
	</form>
</div>
