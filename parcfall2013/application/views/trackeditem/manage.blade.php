<ul class="breadcrumb">
	  ...
	  <li class="active">{{$employee->firstName}} {{$employee->lastName}} Tracked Items</li>
</ul>
				  	...
				  	<div class="control-group ui-widget">
				        ...
				        <div class="controls">
					        <select class="categoryCombobox" name="itemCategory" <%= (typeof viewing !=="undefined" && viewing === true?'disabled="disabled"':'') %>>
					            ...
					            @foreach($trackedcategories as $var)
							 	    <option value="{{ $var->id }}" <%= (typeof trackedcategory_id !=="undefined" && trackedcategory_id==={{$var->id}}? 'selected':'') %>  >{{ $var->categoryname }}</option>
							 	@endforeach
						 	</select>
				        </div>
				    </div>
...
trackedItemSelectList.list = {{Response::eloquent($trackeditems)}};
archivedItemSelectList.list = {{Response::eloquent($archiveditems)}};
var categories = {{Response::eloquent($trackedcategories)}};
			...
			css: {
				...
				onBlock: function () {
					...
					if(c.length>0)
						d['criteria'] = c;
					jQuery.ajax({
						  url: '/trackeditem/read/{{$employee->id}}',
						  type: 'POST',
						  data:d,
						  dataType: 'json',
